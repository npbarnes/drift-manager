...
This Home Manager module aims to be a convenient (and somewhat opinionated), way to handle dotfiles in nix that:
  ...
  3. Provides convenient mechanisms for managing configuration drift. Specifically:
     - a command line tool, `nix-drift` to query changes, revert them, or generate a patch that can be applied to your dotfiles repo.
     - notifications when manual changes have been overwritten with options to revert back to the modified configuration, discard manual configuration, or generate a patch for the changes.
